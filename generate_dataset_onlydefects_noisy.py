...
print(f"\nusing {noisy_backend.backend_name} for defected backend")
...
while True:
    ...
    # adding to dataset
    X = np.load("dataset_onlydefects_noisy_X.npz")['arr_0']
    y = np.load("dataset_onlydefects_noisy_y.npz")['arr_0']
    ...
    np.savez_compressed("dataset_onlydefects_noisy_X", X)
    np.savez_compressed("dataset_onlydefects_noisy_y", y)
